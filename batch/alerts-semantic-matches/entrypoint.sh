...
cd /app
echo "entrypoint: BATCH_CORRIDA=${CORRIDA} BATCH_PARALLEL=${PARALLEL} BATCH_TRACE_LAMBDA_PAYLOADS=${BATCH_TRACE_LAMBDA_PAYLOADS:-1} BATCH_INCLUDE_ZERO_CHUNK=${BATCH_INCLUDE_ZERO_CHUNK:-0} BATCH_NO_CREATED_AT_FILTER=${BATCH_NO_CREATED_AT_FILTER:-0} BATCH_TESTING_EMAIL=${BATCH_TESTING_EMAIL:-} BATCH_PUBLISH_EMAIL_QUEUE=${BATCH_PUBLISH_EMAIL_QUEUE:-0} BATCH_PUBLISH_ALERT_CREATION_QUEUE=${BATCH_PUBLISH_ALERT_CREATION_QUEUE:-0} output=${OUT} trace=${TRACE_PATH}" >&2
